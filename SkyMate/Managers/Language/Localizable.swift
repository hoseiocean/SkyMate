...
extension Localizable where Self: RawRepresentable, Self.RawValue == String {
  ...
  /// The `localized` property constructs a string key based on the raw value
  /// of the entity and retrieves the localized version of that string from
  /// the `LanguageManager`. If no localized version is found, the raw value is
  /// returned.
